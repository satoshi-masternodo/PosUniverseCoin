...
void Shutdown()
{
    ...
    RenameThread("Posuniversecoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Posuniversecoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: posuniversecoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeposuniversecoinamount=<n> " + _("Keep N Posuniversecoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Posuniversecoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Posuniversecoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Posuniversecoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Posuniversecoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Posuniversecoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Posuniversecoin to complete") << "\n";
    ...
    nAnonymizePosuniversecoinAmount = GetArg("-anonymizeposuniversecoinamount", 0);
    if(nAnonymizePosuniversecoinAmount > 999999) nAnonymizePosuniversecoinAmount = 999999;
    if(nAnonymizePosuniversecoinAmount < 2) nAnonymizePosuniversecoinAmount = 2;
    ...
    LogPrintf("Anonymize Posuniversecoin Amount %d\n", nAnonymizePosuniversecoinAmount);
