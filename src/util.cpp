...
int nAnonymizePosuniversecoinAmount = 1000;
...
#else
    const char* pszModule = "Posuniversecoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Posuniversecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Posuniversecoin
    // Mac: ~/Library/Application Support/Posuniversecoin
    // Unix: ~/.Posuniversecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Posuniversecoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "Posuniversecoin";
#else
    // Unix
    return pathRet / ".Posuniversecoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Posuniversecoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "posuniversecoind.pid"));
