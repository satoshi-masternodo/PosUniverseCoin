...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePosuniversecoinAmount"))
        settings.setValue("nAnonymizePosuniversecoinAmount", 1000);
    nAnonymizePosuniversecoinAmount = settings.value("nAnonymizePosuniversecoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizePosuniversecoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizePosuniversecoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizePosuniversecoinAmount:
            return QVariant(nAnonymizePosuniversecoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizePosuniversecoinAmount:
            nAnonymizePosuniversecoinAmount = value.toInt();
            settings.setValue("nAnonymizePosuniversecoinAmount", nAnonymizePosuniversecoinAmount);
            emit AnonymizePosuniversecoinAmountChanged(nAnonymizePosuniversecoinAmount);
            break;
        default:
            break;
        }
